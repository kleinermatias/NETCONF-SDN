...
sub combine_signal_type_and_fec {
    ...
    # Bugzilla 28237 End
    elsif ($signal_type eq 'otu2ex4') {
      if($fec eq 'nofec') { $r = 'odu2e10gex4'; }
      elsif (($fec eq 'ufec7p') || ($fec eq 'gfec') || ($fec eq '0fec')) { $r = 'otu210geopu2ex4'; }
      elsif ($fec eq 'ufec10p') { $r = 'otu2v26210ge2ex4'; }
      elsif ($fec eq 'ufec12p') { $r = 'otu2v26710ge2ex4'; }
      elsif ($fec eq 'ufec13p') { $r = 'otu2v27010ge2ex4'; }
      elsif ($fec eq 'ufec15p') { $r = 'otu2v27310ge2ex4'; }
      elsif ($fec eq 'ufec20p') { $r = 'otu2v28510ge2ex4'; }
    }
    elsif ($signal_type eq 'otu3+') {
    ...
    elsif ($signal_type eq 'cbr_10362079327') {
      $r = 'cbr_10362079327';
    }
    ...
    elsif ($signal_type eq 'cpri_4') { # Bugzilla 39690
      $r = 'cpri_fr';
    }
...
print ("get_syncdsync_muxponder_method() = map_oxuv=$map_oxuv, map_odtu=$map_odtu\n");
  ...
  if(($line_type eq 'otu3') || ($line_type eq 'otu3+') || ($line_type eq 'otu3p2') ||
    ...
    if(($client_type eq 'oc192') || ($client_type eq '10ge7_1') || ($client_type =~ /10gfc_tc/) || # Bug 31054
       ...
       ($client_type eq '1gfc') || ($client_type eq '1ge') || # Bug 38871
       ($client_type eq 'fdr10_iba') ||                       # Bugzilla 39242
       ($client_type =~ /cpri/) ||
       ($client_type eq '10ge') ||                            # Bugzilla 39690
       ($client_type eq 'cbr_10362079327') ) {
...
print("                          PCvalue = $PCvalue, QCvalue = $QCvalue\n");
    ...
    if($mode eq 'trans40') {
      ...
      elsif (($line_type eq '40gep') and ($client_type eq 'otu2ex4')) {
        $PLvalue = $QCvalue;
        $QLvalue = $PCvalue;
      }
      elsif ($sync == 0) {
    ...
    if (($mode eq 'trans10') || ($mode eq 'lb10') || ($mode eq 'kpga10') || ($mode eq 'cbr_kpga10') ||
      ...
      if (($client_type ne 'pbert_8gfc') && ($client_type !~ /gfc_ra/) &&
        ...
        if( ($client_type =~ /gfc_enh/) ||
           ...
           (($client_type =~ /cpri/) && ($map_oxuv_enum == main::TEN_MAP_GMP_LO))) { # Bug 39690
            ($num_pjo_odu2, $num_fs_odu2, $num_c15_odu2) = get_syncdsync_oxu2($client_type, 'otu2', $map_oxuv_enum);
            $num_just_odu2 = $num_fs_odu2==0 ? $num_c15_odu2 : $num_fs_odu2;
        ...
        if (($client_type =~ /iba/) || ($client_type eq '10ge')) {
            ($num_pjo_odu2, $num_fs_odu2, $num_c15_odu2) = get_syncdsync_oxu2($client_type, 'otu2e', $map_oxuv_enum);
            $num_just_odu2 = $num_fs_odu2==0 ? $num_c15_odu2 : $num_fs_odu2;
            if ($line_type eq 'otu3p2') {
                # Special case to make it fit
