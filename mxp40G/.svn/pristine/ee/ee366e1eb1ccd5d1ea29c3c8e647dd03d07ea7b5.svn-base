 ...
 *
 * $Id: ten_hl_config_40g_t41.c,v 1.103 2013/11/12 22:19:26 jccarlis Exp $
 *
 */
...
/****************************************************************/
{
 ...
 /* Bugzilla #26566 Start */
  ...
  if (deskew_ctrl == TEN_SADECO_DESKEW) {
    ...
    /* Bug #29609 - Implement 40G transponder dynamic reprovisioning */
    rtn |= ten_pp40g_misc_pp40g_deskew_disable_t41(module_id_line);
    rtn |= ten_pp40g_misc_pp40g_deskew_disable_t41(module_id_client);
    ...
    /* Bug #29609 - Implement 40G transponder dynamic reprovisioning */
    /* SADECO back to default values, not used in BYPASS mode */
    rtn |= ten_xcon_sadeco_config(dev_id, TEN_MODULE_A, 0x100, 0x1F00, XCON_SONET_SDH_INACTIVE);
    rtn |= ten_xcon_sadeco_config(dev_id, TEN_MODULE_B, 0x100, 0x1F00, XCON_SONET_SDH_INACTIVE);
    rtn |= ten_xcon_sadeco_sdclen(dev_id, TEN_MODULE_A, 0x97E0);
    rtn |= ten_xcon_sadeco_sdclen(dev_id, TEN_MODULE_B, 0x97E0);
    rtn |= ten_xcon_sadeco_sel_data_stream(dev_id, TEN_MODULE_A, 0, 0x1F);
    rtn |= ten_xcon_sadeco_sel_data_stream(dev_id, TEN_MODULE_B, 0, 0x1F);
    rtn |= ten_xcon_sadeco_enable_fp(dev_id, TEN_MODULE_A, CS_DISABLE);
    rtn |= ten_xcon_sadeco_enable_fp(dev_id, TEN_MODULE_B, CS_DISABLE);
  ...
  /* restore defaults from those settings that were left over from 40GEP */
  if (dyn_repro != TEN_INITIAL_CONFIG) {
    ...
    ten_mpif_global_cfg_per_module (dev_id, TEN_MODULE_A, 0, 0); 
    ten_mpif_global_cfg_per_module (dev_id, TEN_MODULE_B, 0, 0); 
...
/****************************************************************/
/* $rtn_hdr_start  HIGH LEVEL CONFIG 40GE TO 40GE TRANSPONDER   */
/* CATEGORY   : API                                             */
/* ACCESS     : Public                                          */
/* BLOCK      : HIGH LEVEL CONFIG                               */
/* CHIP       : Tenabo                                          */
...
/* RETURNS    : CS_OK or CS_ERROR                               */
/* DESCRIPTION:                                                 */
/* Configures 40GE ethernet on the client side into 40GE on     */
/* line side.                                                   */
/*                                                              */
/* [module_id_line] parameter specifies line's module ID        */
/* 16 devices can be registered and module bit indicates line   */
...
/****************************************************************/
{
 ...
 /* Remove hard reset from the top */
  ...
  CS_PRINT("ten_hl_config_40ge_40ge_t41 complete, rtn=%s.\n", (rtn == CS_OK ? "CS_OK" : "CS_ERROR"));
  if (rtn != CS_OK) {
    CS_PRINT("%s() FAILED!\n", func);
  }
...
/****************************************************************/
{
  ...
  if (deskew_ctrl == TEN_SADECO_DESKEW) {
    ...
    /* Bug #29609 - Implement 40G transponder dynamic reprovisioning */
    rtn |= ten_pp40g_misc_pp40g_deskew_disable_t41(module_id);
  } 
  else {
    /* Do the deskewing in the PP40G bypass */
    rtn |= ten_pp40g_rx_pp40g_rx_bypass_t41 (module_id);
