...
------------------------------------------------------------------------
  ...
  the powerTable table.  The agent will start by looking for the right
  ...
  file defaults/table-powerTable.m2d, and then re-running mib2c.
 ...
 Index(es) for the powerTable table
  ------------------------------------------------------------
  The index(es) for the powerTable table are:
...
************************************************************************
powerTable File Overview
------------------------------------------------------------------------
  Several files have been generated to implement the powerTable
  table. We'll go through these files, one by one, explaining each and
  letting you know which you need to edit.


File: powerTable_data_access.[c|h]
------------------------------------------------------------------------
  The powerTable_data_access file contains the interface to your data in
  ...
  FUNC : powerTable_indexes_set
  WHERE: powerTable_data_access.c
...
------------------------------------------------------------------------
  ...
  Set $m2c_table_dependencies = 0 in defaults/table-powerTable.m2d
  ...
  FUNC : powerIndex_check_index


  Validate index
  --------------
  TODO : check that all index components are valid
  FUNC : powerTable_validate_index
...
NOTE: Depending on your configuration, some of the functions used in the
      ...
      were generated for the powerTable table.

To watch the flow of the powerTable table, use the
...
e.g.
        snmpd -f -Le -DpowerTable,verbose:powerTable,internal:powerTable
