...
------------------------------------------------------------------------
  ...
  the txpTempTable table.  The agent will start by looking for the right
  ...
  file defaults/table-txpTempTable.m2d, and then re-running mib2c.
    ...
    will be generated:
      ...
      txpTempTable_allocate_data
      txpTempTable_release_data
 ...
 Index(es) for the txpTempTable table
  ------------------------------------------------------------
  The index(es) for the txpTempTable table are:
...
************************************************************************
txpTempTable File Overview
------------------------------------------------------------------------
  Several files have been generated to implement the txpTempTable
  table. We'll go through these files, one by one, explaining each and
  letting you know which you need to edit.


File: txpTempTable_data_access.[c|h]
------------------------------------------------------------------------
  The txpTempTable_data_access file contains the interface to your data in
  ...
  FUNC : txpTempTable_indexes_set
  WHERE: txpTempTable_data_access.c
...
File: txpTempTable_data_get.c
------------------------------------------------------------------------
  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : txpTempAround_get

  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : txpTempCase_get

  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : txpTempTxLaser_get

  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : txpTempRxLaser_get



File: txpTempTable_data_set.c
------------------------------------------------------------------------
...
NOTE: Depending on your configuration, some of the functions used in the
      ...
      were generated for the txpTempTable table.

To watch the flow of the txpTempTable table, use the
following debug tokens:

        snmp_agent
        helper:table:req
        txpTempTable
        verbose:txpTempTable
        internal:txpTempTable

e.g.
        snmpd -f -Le -DtxpTempTable,verbose:txpTempTable,internal:txpTempTable
...
/*---------------------------------------------------------------------
 * FTNC-MIB::txpTempEntry.txpTempIndex
 * txpTempIndex is subid 1 of txpTempEntry.
 * Its status is Current, and its access level is NoAccess.
 * OID: .1.3.6.1.4.1.43484.2.9.2.1.1.1
 * Description:
 ...
 *
 *
 * NOTE: NODE txpTempIndex IS NOT ACCESSIBLE
 *
 *
 */
/*---------------------------------------------------------------------
