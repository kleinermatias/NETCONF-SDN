 ...
 *
 * $Id: ten_hl_config_otu2_t41.c,v 1.44 2014/04/04 17:22:18 jccarlis Exp $
 *
 */
...
/****************************************************************/
/* $rtn_hdr_start  HIGH LEVEL CONFIG OTU2V TO 40GEP             */
/*                 40G MUXPONDER                                */
/* CATEGORY   : API                                             */
/* ACCESS     : Public                                          */
/* BLOCK      : HIGH LEVEL CONFIG                               */
...
/* RETURNS    : CS_OK or CS_ERROR                               */
/* DESCRIPTION:                                                 */
/* Configures 40GEP on the client side into 4xOTU2e on line side*/
/* NOTE: Since XFI is on module B, client signal must be        */
/*       allocated on Module B.                                 */
/*                                                              */
/* [module_id_line] parameter specifies line's module ID        */
/* 16 devices can be registered and module bit indicates line   */
/* or client side of the registered device. The registered device*/
...
/*   TEN_TRAFFIC_TYPE_PARALLEL_40GE = 42                        */
/*                                                              */
/* [fec_type] parameter defines the fec type:                   */
/*   TEN_FEC_MODE_OTUkV             = 0  (7  %)                 */
/*   TEN_FEC_MODE_OTUkV_262         = 1  (10 %)                 */
/*   TEN_FEC_MODE_OTUkV_273         = 2  (15 %)                 */
/*   TEN_FEC_MODE_OTUkV_285         = 3  (20 %)                 */
...
/****************************************************************/
{
    ...
    if ((traffic_type_line == TEN_TRAFFIC_TYPE_PARALLEL_40GE) && (traffic_type_client != TEN_TRAFFIC_TYPE_OTU2EX4)) {
        CS_HNDL_ERROR(module_id_line, ETEN_MOD_INVALID_USER_ARG, "%s() traffic_type_client %d out of range.  s/b 6, 42.", func, traffic_type_client);
        return (CS_ERROR);
    }

    if ((traffic_type_client == TEN_TRAFFIC_TYPE_OTU2EX4) && (traffic_type_line != TEN_TRAFFIC_TYPE_PARALLEL_40GE)) {
        CS_HNDL_ERROR(module_id_line, ETEN_MOD_INVALID_USER_ARG, "%s() traffic_type_line %d out of range.  s/b 6, 42.", func, traffic_type_line);
        return (CS_ERROR);
    }

    if (fec_type >= TEN_FEC_MODE_MAX_NUM) {
        CS_HNDL_ERROR(module_id_line, ETEN_MOD_INVALID_USER_ARG, "%s() fec_type %d out of range.  s/b 0..15.", func, fec_type);
        return (CS_ERROR);
    }
  ...
  if (deskew_ctrl > TEN_PP40G_DESKEW) {
 ...
 #if 0
    CS_PRINT("%s() %d, dyn=%d, %d, %d, %d\n",
            func,
            module_id_line, dyn_repro,
            traffic_type_line, traffic_type_client,
            fec_type);
 #endif
    CS_PRINT("%s() \n", func);
    CS_PRINT("    line (%s, %s)\n",  ten_module_strings[module_id_line&1], 
    ...
    rtn |= ten_mpif_global_cfg(dev_id, 1, 1, 1, 1);
    rtn |= ten_mpif_global_clock_disable_n40g (dev_id, module_id_line, CS_TX_AND_RX, CS_ENABLE);      
    rtn |= ten_mpif_global_reset_n40g (dev_id, module_id_line, CS_TX_AND_RX, CS_RESET_DEASSERT);
    ncols = ten_hl_n10g_get_ncols(fec_type);
    rtn |= ten_n40g_gbl4x_synccfg (module_id_line, ncols * 16, 1);
                                  
    if ((dyn_repro == TEN_INITIAL_CONFIG) || (dyn_repro == TEN_REPRO_LINE_AND_CLIENT)) {
        rtn |= ten_mpif_global_clock_disable_n40g (dev_id, module_id_client, CS_TX_AND_RX, CS_ENABLE);      
        rtn |= ten_mpif_global_reset_n40g (dev_id, module_id_client, CS_TX_AND_RX, CS_RESET_DEASSERT);

        ten_n40g_rx_pkt_bypass_t41 (module_id_client);  /* OBY=1, SBY=1 */
        ten_n40g_tx_pkt_bypass_t41 (module_id_client);  /* OBY=1, SBY=1, KPGBY=1 */
    }

    for(slice=0; slice<TEN_MAX_SLICES; slice++) {
        rtn |= ten_hl_n10g_config_t41 (module_id_line, 
                                       slice,
                                       dyn_repro,
        ...
        rtn |= ten_n10g_tx_pp40g_enable_t41 (module_id_line, slice);            /* PP40G_EN = 1 */
        
        /* Frame-synchronize the four synchronous Tx OxU2(V) to allow reassembly 
           of the generic split signals at the remote end */
        rtn |= ten_n10g_otnt_set_mst_fpsync (module_id_line, slice, 1);         /* MST_FPSYNC = 1 */
        
        /* Clear all justification control bits since OPU2e is fast enough to handle 1/4th 40GEP */
        rtn |= ten_n10g_otnt_set_jc_odtu23 (module_id_line, slice, 0, 0, 0);    /* JCs = 0*/
    }

    if ((dyn_repro == TEN_INITIAL_CONFIG) || (dyn_repro == TEN_REPRO_LINE_AND_CLIENT)) {
        rtn |= ten_hl_pp40g_config_t41(module_id_client,
    ...
    rtn |= ten_hl_pp40g_config_t41(module_id_line,
                                   TEN_PP40G_RX_40GEP_XCON,
                                   TEN_PP40G_TX_XCON_40GEP,
                                   deskew_ctrl,
                                   sysclk);

    /* Disable the frame pulse generation from line TX PP40G ad do it instead in N10G using MST_FPSYNC */
    rtn |= ten_pp40g_ctrl_tx_subblks_t41(module_id_line, 
                                         TEN_PP40G_TX_BLK_PCS_FPG, 
                                         CS_DISABLE);

    /* While skew on the line may be small, it is possible for the deskew block to see a larger skew. 
       This can occur if the 40GBASE-R signal is mapped over OTN with virtual lanes mapped into separate 
       ODU2e/OTU2e payloads and the ODU2e/OTU2e/FEC overhead slots are not aligned. If this is the case, 
       the value of skew threshold must be increased by the width of ODU2e/OTU2e/FEC overhead slot to compensate.*/
    if((fec_type != TEN_FEC_MODE_ZEROFEC) && (fec_type != TEN_FEC_MODE_NOFEC)) {
        npar = ten_hl_n10g_get_npar(fec_type);
        rtn |= ten_pp40g_set_pcs_rx_skew_thresh_t41(module_id_line, npar*4*8);      /* 4 rows x 8 bits */
    }
    
    for(slice=0;slice<TEN_MAX_SLICES;slice++) {
        rtn |= ten_hl_xcon_config(dev_id, (module_id_line*4)+slice, (module_id_client*4)+slice, TEN_TRAFFIC_TYPE_10GE);
        rtn |= ten_hl_xcon_config(dev_id, (module_id_client*4)+slice,(module_id_line*4)+slice, TEN_TRAFFIC_TYPE_10GE);
        rtn |= ten_hl_config_fec_t41 (module_id_line, slice, TEN_SLICE_10G, fec_type);
    }
    
    for(slice=0; slice<TEN_MAX_SLICES; slice++) {
      rtn |= ten_n10g_tx_circuit_bypass_t41 (module_id_client, slice);    /* PKTIE=0, TX_BYPASS=1, CLKTX_DIS=1 */
      rtn |= ten_n10g_rx_bypass_external_t41 (module_id_client, slice );  /* RX_BYPASS10G=1, RX_BYPASS40G=0 */
    }
