...
my %traffic_type_enum = (
    ...
    'otu2ex4'              => main::TEN_TRAFFIC_TYPE_OTU2EX4,
    ...
    'cbr_10362079327'      => main::TEN_TRAFFIC_TYPE_CBR_10G,
    ...
    'cpri_4'               => main::TEN_TRAFFIC_TYPE_CPRI_4, # Bugzilla 39690
...
sub check_pp40g_pcs_mono_insync{
    my ($module_id, $slice) = @_;
    printf "check_pp40g_pcs_mono_insync   %d ... ", $module_id;
...
sub check_pp40g_pcs_lane_insync{
    my ($module_id, $slice) = @_;
    printf "check_pp40g_pcs_lane_insync   %d ... ", $module_id;
...
sub check_all {
    ...
    if (($args{client} ne 'null') &&
        ...
        if ($args{hsif_client} eq 'xfi') {
            if ($args{clientrate} == 40) {
              printf "check_xfi_filt_lock      %s (TEN_SLICE_ALL) ... ", module_only($mod_client);
    ...
    if ($args{linerate} == 40) {
      if ($args{hsif_line} eq 'cfp_mr') {
        printf "check_cfp_mr_filt_lock   %s (TEN_SLICE_ALL) ... ", module_only($mod_line);
        # Should return 0 on a PASS
        if (check_cfp_mr_filt_lock_40g($mod_line) != 0) {
          print "FAIL *\n";
          $fail++;
        }
        else {
...
#    if (($args{line} eq 'otu2') && ($args{client} !~/pbert/) && ($args{dyn_repro} != main::TEN_REPRO_LINE_AND_CLIENT)) {
    ...
    if (($args{mode} eq 'trans40') && ($args{client} ne 'null')) {  # Bugzilla 29609
        ...
        ###############################################################################
        if ($args{line} eq 'otu2ex4') {
           $fail += check_n10g_otn_inframe($mod_line, 0);
           $fail += check_n10g_otn_inframe($mod_line, 1);
           $fail += check_n10g_otn_inframe($mod_line, 2);
           $fail += check_n10g_otn_inframe($mod_line, 3);
        }
        elsif ($args{line} eq '40gep') {
            $fail += check_pp40g_pcs_lane_insync($mod_line);
        }
        elsif (check_n40g_otn_inframe ($mod_line)) {
...
#########################
    -client             str - Traffic Type on the client side (default: invalid)
                              ...
                              <otu2e>               Overclocked OTU2(OTU2e) with 10GE Client
                              <cbr_10362079327>     10G stream @ (836/832) * Ethernet LAN
                              <10ge>                10G Ethernet LAN (for mapping into OTU2e)
...
$odtu_map_string
    ...
    -pre_line_rx/        str - PP40G Pre-Line RX&TX/Pre-Client RX&TX
    -pre_line_tx/              <40gep>
    -pre_client_rx/            <40gesbi>
    -pre_client_tx/            <40ges6466>
                               <40gex>
                               <40gep_ra>
                               <40gesbi_ra>
                               <40ges6466_ra>
...
sub validate_client {
    ...
    if (($client eq 'none') or      # Don't change it (default for $dyn_repro == 1)
        ...
        ($client eq '1gfc') or
        ($client eq '2gisc') or
        ($client eq 'cpri_4') or # Bugzilla 39690
...
sub validate_line {
    ...
    if (($line eq 'none') or      # Don't change it (default for $dyn_repro == 1)
        ...
        ($line eq '5g_iba') or  
        ($line eq '10g_iba') or
        ($line eq '1ge') or # Bugzilla 39690
        ($line eq 'cpri_4') or # Bugzilla 39690
        ($line eq 'cpri_7') or # Bugzilla 39690
        ($line eq '2gfc') or # Bugzilla 39690
        ($line eq '4gfc') or # Bugzilla 39690
        ($line eq 'fdr10_iba') or # Bugzilla 39242
        ($line eq 'otu2ex4')) {
        return;
    }
    else {
...
sub validate_line_client {
    ...
    if (($args{mode} eq 'mux') || ($args{mode} eq 'trans10') || ($args{mode} eq 'trans40')) {
        ...
        if ($args{client} eq 'otu1e') {
            if ($args{linerate} == 40) {
                if (($args{line} ne 'otu3p2') && ($args{line} ne 'otu3e') && ($args{line} ne 'otu3e4')) {
...
sub check_args {
    ...
    while (my $arg = shift @ARGV) {
      ...
      if((exists $args{client}) && ($args{client} ne 'cbr_1118099736') && (exists $args{line}) && ($args{line} ne 'cbr_1052329163')){
        ...
        elsif ($arg eq '-pre_line_rx') {
            $args{pre_line_rx} = shift @ARGV;
            unless (exists $ten_40ge_rx_pre_enum{$args{pre_line_rx}}) {
                die "Invalid -pre_line_rx $args{pre_line_rx} invalid selection.\n";
            }
            $args{pre_line_rx_enum} = $ten_40ge_rx_pre_enum{$args{pre_line_rx}};
            $pp40g=$args{pre_line_rx};
            $pp40g=~s/_ra//;
            $args{pre_line} = $pp40g;
        }
        elsif ($arg eq '-pre_line_tx') {
            $args{pre_line_tx} = shift @ARGV;
            unless (exists $ten_40ge_tx_pre_enum{$args{pre_line_tx}}) {
                die "Invalid -pre_line_tx $args{pre_line_tx} invalid selection.\n";
            }
            $args{pre_line_tx_enum} = $ten_40ge_tx_pre_enum{$args{pre_line_tx}};
            $pp40g=$args{pre_line_tx};
            $pp40g=~s/_ra//;
            $args{pre_line} = $pp40g;
        }
        ...
        elsif ($arg eq '-pre_client_rx') {
            $args{pre_client_rx} = shift @ARGV;
            unless (exists $ten_40ge_rx_pre_enum{$args{pre_client_rx}}) {
                die "Invalid -pre_client_rx $args{pre_client_rx} invalid selection.\n";
            }
            $args{pre_client_rx_enum} = $ten_40ge_rx_pre_enum{$args{pre_client_rx}};
            $pp40g=$args{pre_client_rx};
            $pp40g=~s/_ra//;
            $args{pre_client} = $pp40g;
        }
        elsif ($arg eq '-pre_client_tx') {
            $args{pre_client_tx} = shift @ARGV;
            unless (exists $ten_40ge_tx_pre_enum{$args{pre_client_tx}}) {
                die "Invalid -pre_client_tx $args{pre_client_tx} invalid selection.\n";
            }
            $args{pre_client_tx_enum} = $ten_40ge_tx_pre_enum{$args{pre_client_tx}};
            $pp40g=$args{pre_client_tx};
            $pp40g=~s/_ra//;
            $args{pre_client} = $pp40g;
        }
        # Bugzilla 28006 End
