 ...
 *
 * $Id: ten_mpif.c,v 1.63 2014/04/03 20:03:22 jccarlis Exp $
 *
 */
...
/****************************************************************/
{
  ...
  start_tics        = (cs_uint32)(TEN_REG_READ(TEN_MPIF_REG_ADDR(pDev, MISC_COUNTER1)) << 16);
  start_tics       |= (cs_uint32)TEN_REG_READ(TEN_MPIF_REG_ADDR(pDev, MISC_COUNTER0));
  for (rd_cnt=0; rd_cnt<(TEN_MPIF_UDELAY_HI_RES_TST); rd_cnt++) {
    TEN_REG_READ(TEN_MPIF_REG_ADDR(pDev, MISC_COUNTER3));
    TEN_REG_READ(TEN_MPIF_REG_ADDR(pDev, MISC_COUNTER2));
    /* Bugzilla #42300 - Fix klocwork warnings */
    end_tics        = (cs_uint32)(TEN_REG_READ(TEN_MPIF_REG_ADDR(pDev, MISC_COUNTER1)) << 16);
    end_tics       |= (cs_uint32)TEN_REG_READ(TEN_MPIF_REG_ADDR(pDev, MISC_COUNTER0));
  }
  g_pTEN->drvr_flag &= (~TEN_DRVR_LOG_SUSPEND | log_suspend_flag);   /* restore log suspend flag */
  TEN_DEV_UNLOCK(dev_id, TEN_ID_MPIF);
...
static cs_uint32 ten_mpif_get_misc_counter_low32(ten_dev_cb_t *pdevcb, cs_uint16 read_type)
{
  ...
  switch (read_type) {
    ...
    case TEN_MPIF_UDELAY_MODE_HI_LONG :
      ...
      /* Bugzilla #42300 - Fix klocwork warnings */
      time  = (cs_uint32)TEN_REG_READ(TEN_MPIF_REG_ADDR(pDev, MISC_COUNTER1));
      time  = (cs_uint32)((time << 16) & 0xFFFF0000);
      time |= (cs_uint32)TEN_REG_READ(TEN_MPIF_REG_ADDR(pDev, MISC_COUNTER0));
