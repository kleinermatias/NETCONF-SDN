...
------------------------------------------------------------------------
  ...
  the txpBandTable table.  The agent will start by looking for the right
  ...
  file defaults/table-txpBandTable.m2d, and then re-running mib2c.
    ...
    will be generated:
      ...
      txpBandTable_allocate_data
      txpBandTable_release_data
 ...
 Index(es) for the txpBandTable table
  ------------------------------------------------------------
  The index(es) for the txpBandTable table are:
...
************************************************************************
txpBandTable File Overview
------------------------------------------------------------------------
  Several files have been generated to implement the txpBandTable
  table. We'll go through these files, one by one, explaining each and
  letting you know which you need to edit.


File: txpBandTable_data_access.[c|h]
------------------------------------------------------------------------
  The txpBandTable_data_access file contains the interface to your data in
  ...
  FUNC : txpBandTable_indexes_set
  WHERE: txpBandTable_data_access.c
...
File: txpBandTable_data_get.c
------------------------------------------------------------------------
  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : txpTxLaserITUBand_get

  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : txpRxLaserITUBand_get



File: txpBandTable_data_set.c
------------------------------------------------------------------------
...
NOTE: Depending on your configuration, some of the functions used in the
      ...
      were generated for the txpBandTable table.

To watch the flow of the txpBandTable table, use the
following debug tokens:

        snmp_agent
        helper:table:req
        txpBandTable
        verbose:txpBandTable
        internal:txpBandTable

e.g.
        snmpd -f -Le -DtxpBandTable,verbose:txpBandTable,internal:txpBandTable
...
/*---------------------------------------------------------------------
 * FTNC-MIB::txpBandEntry.txpBandIndex
 * txpBandIndex is subid 1 of txpBandEntry.
 * Its status is Current, and its access level is NoAccess.
 * OID: .1.3.6.1.4.1.43484.2.9.2.4.1.1
 * Description:
 ...
 *
 *
 * NOTE: NODE txpBandIndex IS NOT ACCESSIBLE
 *
 *
 */
/*---------------------------------------------------------------------
