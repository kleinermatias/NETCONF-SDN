 ...
 *
 * $Id: ten_n10g_sdfr.c,v 1.7 2013/02/22 20:47:56 jccarlis Exp $
 *
 */
...
/****************************************************************/
/* $rtn_hdr_start  N10G GET SONET/SDH RECEIVER STATUS           */
/* CATEGORY   : API                                             */
/* ACCESS     : Public                                          */
/* BLOCK      : N10G                                            */
/* CHIP       : Tenabo                                          */
cs_int16 ten_n10g_get_sdfr_rx_status(cs_uint16 module_id,
                                     cs_uint8 slice,
                                     cs_uint16 field)
/* INPUTS     : o Module Id                                     */
/*              o Slice                                         */
...
/****************************************************************/
{
  ...
  TEN_N10G_SDFR_SDFSTAT_t tmp_sdfr_sdfstat;
  cs_char8 *func = "ten_n10g_get_sdfr_rx_status";  
  ...
  CS_PRINT("%s() %s, %s field=%d\n", func, ten_module_strings[module_id&1], ten_slice_strings[slice], field);
  
  
  tmp_sdfr_sdfstat.wrd = TEN_REG_READ(TEN_N10G_SDFR_REG_ADDR(pDev, module_id, slice, SDFSTAT));
  switch (field) {
  ...
  default :
    {
      CS_HNDL_ERROR(module_id, ETEN_MOD_INVALID_USER_ARG, "%s() field %d out of range.  s/b 0..6, 0xFF.", func, field);
      rtn = CS_ERROR;
      break;
    } 
  }
...
/****************************************************************/
/* $rtn_hdr_start  N10G SET LOSS-OF-FRAME TIME SPAN CONFIG REG  */
/* CATEGORY   : API                                             */
/* ACCESS     : Public                                          */
/* BLOCK      : N10G                                            */
/* CHIP       : Tenabo                                          */
cs_status ten_n10g_set_sdfr_lofcfg1(cs_uint16 module_id, 
                                    cs_uint8 slice, 
                                    cs_uint16 lofimou, 
                                    cs_uint16 lofimin)
/* INPUTS     : o Module Id                                     */
...
{
  cs_uint16 sl;
  TEN_N10G_SDFR_LOFCFG1_t tmp_sdfr_lofcfg1;
  T41_t *pDev = NULL;
  ...
  for (sl=0; sl<TEN_MAX_SLICES; sl++) {
    if ((slice == sl) || (slice == TEN_SLICE_ALL)) {
      tmp_sdfr_lofcfg1.wrd = TEN_REG_READ(TEN_N10G_SDFR_REG_ADDR(pDev, module_id, sl, LOFCFG1));
      tmp_sdfr_lofcfg1.bf.LOFIMOU = lofimou;
      tmp_sdfr_lofcfg1.bf.LOFIMIN = lofimin;
      TEN_REG_WRITE(TEN_N10G_SDFR_REG_ADDR(pDev, module_id, sl, LOFCFG1), tmp_sdfr_lofcfg1.wrd);
    }
  }
...
/****************************************************************/
/* $rtn_hdr_start  N10G SET B1 BIT ERROR THRESHOLD REGISTER     */
/* CATEGORY   : API                                             */
/* ACCESS     : Public                                          */
/* BLOCK      : N10G                                            */
/* CHIP       : Tenabo                                          */
cs_status ten_n10g_set_sdfr_b1btber(cs_uint16 module_id, 
                                    cs_uint8 slice, 
                                    cs_uint16 b1btber)
/* INPUTS     : o Module Id                                     */
/*              o B1BTBER                                       */
...
{
  cs_uint16 sl;
  TEN_N10G_SDFR_B1BTBER_t tmp_sdfr_b1btber;
  T41_t *pDev = NULL;
  ...
  for (sl=0; sl<TEN_MAX_SLICES; sl++) {
    if ((slice == sl) || (slice == TEN_SLICE_ALL)) {
      tmp_sdfr_b1btber.wrd = TEN_REG_READ(TEN_N10G_SDFR_REG_ADDR(pDev, module_id, sl, B1BTBER));
      tmp_sdfr_b1btber.bf.B1BTBER = b1btber;
      TEN_REG_WRITE(TEN_N10G_SDFR_REG_ADDR(pDev, module_id, sl, B1BTBER), tmp_sdfr_b1btber.wrd);
    }
  }
  TEN_MOD_UNLOCK(module_id, TEN_ID_N10G);
  return (CS_OK);
...
/****************************************************************/
/* $rtn_hdr_start  N10G SET B1 BLOCK LEVEL ERROR ERROR THRESHOLD*/
/* CATEGORY   : API                                             */
/* ACCESS     : Public                                          */
/* BLOCK      : N10G                                            */
/* CHIP       : Tenabo                                          */
cs_status ten_n10g_set_sdfr_b1blber(cs_uint16 module_id, 
                                    cs_uint8 slice, 
                                    cs_uint16 b1blber)
/* INPUTS     : o Module Id                                     */
/*              o B1BLBER                                       */
...
{
  cs_uint16 sl;
  TEN_N10G_SDFR_B1BLBER_t tmp_sdfr_b1blber;
  T41_t *pDev = NULL;
  ...
  for (sl=0; sl<TEN_MAX_SLICES; sl++) {
    if ((slice == sl) || (slice == TEN_SLICE_ALL)) {
      tmp_sdfr_b1blber.wrd = TEN_REG_READ(TEN_N10G_SDFR_REG_ADDR(pDev, module_id, sl, B1BLBER));
      tmp_sdfr_b1blber.bf.B1BLBER = b1blber;
      TEN_REG_WRITE(TEN_N10G_SDFR_REG_ADDR(pDev, module_id, sl, B1BLBER), tmp_sdfr_b1blber.wrd);
    }
  }
  TEN_MOD_UNLOCK(module_id, TEN_ID_N10G);
  return (CS_OK);
...
/****************************************************************/
/* $rtn_hdr_start  N10G SET B2 BIT ERROR THRESHOLD REGISTER     */
/* CATEGORY   : API                                             */
/* ACCESS     : Public                                          */
/* BLOCK      : N10G                                            */
/* CHIP       : Tenabo                                          */
cs_status ten_n10g_set_sdfr_b2btber(cs_uint16 module_id, 
                                    cs_uint8 slice, 
                                    cs_uint16 b2btber0,
                                    cs_uint16 b2btber1)
...
{
  cs_uint16 sl;
  TEN_N10G_SDFR_B2BTBER0_t tmp_sdfr_b2btber0;
  TEN_N10G_SDFR_B2BTBER1_t tmp_sdfr_b2btber1;
  T41_t *pDev = NULL;
  ...
  for (sl=0; sl<TEN_MAX_SLICES; sl++) {
    if ((slice == sl) || (slice == TEN_SLICE_ALL)) {
      tmp_sdfr_b2btber0.wrd = TEN_REG_READ(TEN_N10G_SDFR_REG_ADDR(pDev, module_id, sl, B2BTBER0));
      tmp_sdfr_b2btber0.bf.B2BTBER0 = b2btber0;
      TEN_REG_WRITE(TEN_N10G_SDFR_REG_ADDR(pDev, module_id, sl, B2BTBER0), tmp_sdfr_b2btber0.wrd);
      tmp_sdfr_b2btber1.wrd = TEN_REG_READ(TEN_N10G_SDFR_REG_ADDR(pDev, module_id, sl, B2BTBER1));
      tmp_sdfr_b2btber1.bf.B2BTBER1 = b2btber1;
      TEN_REG_WRITE(TEN_N10G_SDFR_REG_ADDR(pDev, module_id, sl, B2BTBER1), tmp_sdfr_b2btber1.wrd);
    }
  }
  TEN_MOD_UNLOCK(module_id, TEN_ID_N10G);
  return (CS_OK);
...
/****************************************************************/
/* $rtn_hdr_start  N10G SET B2 BLOCK LEVEL ERROR ERROR THRESHOLD*/
/* CATEGORY   : API                                             */
/* ACCESS     : Public                                          */
/* BLOCK      : N10G                                            */
/* CHIP       : Tenabo                                          */
cs_status ten_n10g_set_sdfr_b2blber(cs_uint16 module_id, 
                                    cs_uint8 slice, 
                                    cs_uint16 b2blber0,
                                    cs_uint16 b2blber1)
/* INPUTS     : o Module Id                                     */
...
{
  cs_uint16 sl;
  TEN_N10G_SDFR_B2BLBER0_t tmp_sdfr_b2blber0;
  TEN_N10G_SDFR_B2BLBER1_t tmp_sdfr_b2blber1;
  T41_t *pDev = NULL;
  ...
  for (sl=0; sl<TEN_MAX_SLICES; sl++) {
    if ((slice == sl) || (slice == TEN_SLICE_ALL)) {
      tmp_sdfr_b2blber0.wrd = TEN_REG_READ(TEN_N10G_SDFR_REG_ADDR(pDev, module_id, sl, B2BLBER0));
      tmp_sdfr_b2blber0.bf.B2BLBER0 = b2blber0;
      TEN_REG_WRITE(TEN_N10G_SDFR_REG_ADDR(pDev, module_id, sl, B2BLBER0), tmp_sdfr_b2blber0.wrd);
      tmp_sdfr_b2blber1.wrd = TEN_REG_READ(TEN_N10G_SDFR_REG_ADDR(pDev, module_id, sl, B2BLBER1));
      tmp_sdfr_b2blber1.bf.B2BLBER1 = b2blber1;
      TEN_REG_WRITE(TEN_N10G_SDFR_REG_ADDR(pDev, module_id, sl, B2BLBER1), tmp_sdfr_b2blber1.wrd);
    }
  }
  TEN_MOD_UNLOCK(module_id, TEN_ID_N10G);
  return (CS_OK);
...
/****************************************************************/
/* $rtn_hdr_start  N10G SDFR SET SDFCFG  REGISTER               */
/* CATEGORY   : API                                             */
/* ACCESS     : Public                                          */
/* BLOCK      : N10G                                            */
/* CHIP       : Tenabo                                          */
...
{
  cs_uint16 sl;
  TEN_N10G_SDFR_SDFCFG_t tmp_sdfr_sdfcfg;
  T41_t *pDev = NULL;
  ...
  for (sl=0; sl<TEN_MAX_SLICES; sl++) {
    if ((slice == sl) || (slice == TEN_SLICE_ALL)) {
      tmp_sdfr_sdfcfg.wrd = TEN_REG_READ(TEN_N10G_SDFR_REG_ADDR(pDev, module_id, sl, SDFCFG));
      ...
      TEN_REG_WRITE(TEN_N10G_SDFR_REG_ADDR(pDev, module_id, sl, SDFCFG), tmp_sdfr_sdfcfg.wrd);
    }
  }
  TEN_MOD_UNLOCK(module_id, TEN_ID_N10G);
...
/****************************************************************/
/* $rtn_hdr_start  N10G SDFR SET SCREN                          */
/* CATEGORY   : API                                             */
/* ACCESS     : Public                                          */
/* BLOCK      : N10G                                            */
/* CHIP       : Tenabo                                          */
...
/****************************************************************/
/* $rtn_hdr_start  N10G SDFR SET SDFCFG1 REGISTER               */
/* CATEGORY   : API                                             */
/* ACCESS     : Public                                          */
/* BLOCK      : N10G                                            */
/* CHIP       : Tenabo                                          */
...
{
  cs_uint16 sl;
  TEN_N10G_SDFR_SDFCFG1_t tmp_sdfr_sdfcfg1;
  T41_t *pDev = NULL;
  ...
  for (sl=0; sl<TEN_MAX_SLICES; sl++) {
    if ((slice == sl) || (slice == TEN_SLICE_ALL)) {
      tmp_sdfr_sdfcfg1.wrd = TEN_REG_READ(TEN_N10G_SDFR_REG_ADDR(pDev, module_id, sl, SDFCFG1));
      tmp_sdfr_sdfcfg1.bf.ALTSCR = altscr;
      TEN_REG_WRITE(TEN_N10G_SDFR_REG_ADDR(pDev, module_id, sl, SDFCFG1), tmp_sdfr_sdfcfg1.wrd);
    }
  }
  TEN_MOD_UNLOCK(module_id, TEN_ID_N10G);
...
/****************************************************************/
/* $rtn_hdr_start  N10G SDFR SELECT GLOBAL TIMING SOURCE        */
/* CATEGORY   : API                                             */
/* ACCESS     : Public                                          */
/* BLOCK      : N10G                                            */
/* CHIP       : Tenabo                                          */
...
{
  cs_uint16 sl;
  TEN_N10G_SDFR_LOFCFG2_t tmp_sdfr_lofcfg2;
  T41_t *pDev = NULL;
  ...
  for (sl=0; sl<TEN_MAX_SLICES; sl++) {
    if ((slice == sl) || (slice == TEN_SLICE_ALL)) {
      tmp_sdfr_lofcfg2.wrd = TEN_REG_READ(TEN_N10G_SDFR_REG_ADDR(pDev, module_id, sl, LOFCFG2));
      tmp_sdfr_lofcfg2.bf.EOFGCFG = select;
      TEN_REG_WRITE(TEN_N10G_SDFR_REG_ADDR(pDev, module_id, sl, LOFCFG2), tmp_sdfr_lofcfg2.wrd);
    }
  }
  TEN_MOD_UNLOCK(module_id, TEN_ID_N10G);
