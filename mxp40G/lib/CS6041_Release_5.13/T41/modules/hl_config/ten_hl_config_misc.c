 ...
 *
 * $Id: ten_hl_config_misc.c,v 1.75 2014/04/02 19:11:48 pptawade Exp $
 *
 */
...
/*  cs_uint8 actual_slice_line; */
  ...
  if ((line == TEN_TRAFFIC_TYPE_OTU2) ||
      ...
      (line == TEN_TRAFFIC_TYPE_CPRI_4) || /* Bugzilla 39690 */
      ...
      (line == TEN_TRAFFIC_TYPE_CBR_10G)) {     /* Bugzilla 43573 */
...
/****************************************************************/
{
  ...
  cs_uint16 temp_mod_num_line = TEN_MOD_ID_TO_MOD_NUM(module_id_line);
  cs_uint16 temp_mod_num_client = TEN_MOD_ID_TO_MOD_NUM(module_id_client);
  cs_status rtn = CS_OK;
  ...
  /* Bug #41646 - pbert_10ge mode has 8 bus errors on initial config  */
  if ((dyn_repro == TEN_INITIAL_CONFIG) || (dyn_repro == TEN_REPRO_LINE_AND_CLIENT)) {
    ...
    rtn |= ten_mpif_global_clock_disable_n40g (temp_dev_id, temp_mod_num_line, CS_RX, CS_ENABLE);
    rtn |= ten_mpif_global_reset_n40g (temp_dev_id, temp_mod_num_line, CS_RX, CS_RESET_DEASSERT);
  
    /* Start clocking and take out of reset the N40G TX block */
    rtn |= ten_mpif_global_clock_disable_n40g (temp_dev_id, temp_mod_num_line, CS_TX, CS_ENABLE);
    rtn |= ten_mpif_global_reset_n40g (temp_dev_id, temp_mod_num_line, CS_TX, CS_RESET_DEASSERT);
    
    /* Start clocking and take out of reset the N40G RX block */
    rtn |= ten_mpif_global_clock_disable_n40g (temp_dev_id, temp_mod_num_client, CS_RX, CS_ENABLE);
    rtn |= ten_mpif_global_reset_n40g (temp_dev_id, temp_mod_num_client, CS_RX, CS_RESET_DEASSERT);
  
    /* Start clocking and take out of reset the N40G TX block */
    rtn |= ten_mpif_global_clock_disable_n40g (temp_dev_id, temp_mod_num_client, CS_TX, CS_ENABLE);
    rtn |= ten_mpif_global_reset_n40g (temp_dev_id, temp_mod_num_client, CS_TX, CS_RESET_DEASSERT);
  }   
