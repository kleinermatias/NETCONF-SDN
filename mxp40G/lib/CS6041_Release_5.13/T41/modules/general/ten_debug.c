 ...
 * testing purposes.
 *
 * $Id: ten_debug.c,v 1.56 2013/07/04 13:23:38 jccarlis Exp $
 *
 */
...
{
  cs_uint16 data;
  cs_uint32 mapped_address;
  #ifdef TEN_CHECK_FOR_BUS_ERRORS
  TEN_MPIF_MPIF_INTERRUPT_t tmp_mpif_interrupt;
  #endif
  
  mapped_address = ((cs_uint32)(pReg) - g_pTEN->debug_base_address)>>1;
  data = CS_REG_READ(&mapped_address);
  ...
  */
  ...
  if (g_pTEN->debug_dev_id >= TEN_MAX_NUM_DEVICES) {
    g_pTEN->debug_dev_id = 0;
  }
  
  #ifdef TEN_CHECK_FOR_BUS_ERRORS
  if (g_pTEN->ten_bus_error_ctl[g_pTEN->debug_dev_id] == CS_ENABLE) {
    ...
    mapped_address =  TEN_MPIF_MPIF_INTERRUPT;
	
	tmp_mpif_interrupt.wrd = CS_REG_READ(&mapped_address);
    if (tmp_mpif_interrupt.bf.buserri) { 
      g_pTEN->ten_bus_error_count[g_pTEN->debug_dev_id]++; /* Bugzilla #26434 */
        CS_PRINT("BUS ERROR ON RD: dev_id=%d, 0x%05X = 0x%04X\n", g_pTEN->debug_dev_id, ((cs_uint32)(pReg) - g_pTEN->debug_base_address)>>1 , data);
        CS_HNDL_ERROR(g_pTEN->debug_dev_id, ETEN_DEV_REG_ACCESS_TIMEOUT, ": Bus Error.");
      ...
      CS_REG_WRITE(&mapped_address, tmp_mpif_interrupt.bf.buserri);
    }
  }
  #endif
...
void TEN_REG_WRITE(volatile cs_uint16 *addr, cs_uint16 data)
{
  ...
  /* Search for dev_id based on addr. */
  cs_uint16 ii;
  cs_uint16 *pDev=0;
  ...
  if (g_pTEN->ws_ctl.enbl == FALSE) {
	...
	mapped_address = ((cs_uint32)(addr) - g_pTEN->debug_base_address)>>1;
    CS_REG_WRITE(&mapped_address, data);
  }
  ...
  if (g_pTEN->ten_bus_error_ctl[g_pTEN->debug_dev_id] == CS_ENABLE) {
   ...
   mapped_address =  TEN_MPIF_MPIF_INTERRUPT;
    ...
    if (tmp_mpif_interrupt.bf.buserri) { 
      g_pTEN->ten_bus_error_count[g_pTEN->debug_dev_id]++; /* Bugzilla #26434 */
      /* if (!(g_pTEN->drvr_flag & TEN_DRVR_LOG_SUSPEND)) { */
        CS_PRINT("BUS ERROR ON WR: dev_id=%d, 0x%05X = 0x%04X\n", g_pTEN->debug_dev_id, ((cs_uint32)(addr) - g_pTEN->debug_base_address)>>1 , (data));
        CS_HNDL_ERROR(g_pTEN->debug_dev_id, ETEN_DEV_REG_ACCESS_TIMEOUT, ": Bus Error.");
      /* } */
      /* Bugzilla #29271, fix bus error warning */
      CS_REG_WRITE(&mapped_address, tmp_mpif_interrupt.bf.buserri);
...
cs_status TEN_DEV_COMMON_VALIDATE_INTERNAL(cs_uint16 DEV_ID, T41_t **BASE_ADDR)
{
  ...
  *BASE_ADDR = (T41_t *)((g_pTEN->p_dev_cb_tbl[DEV_ID])->base_addr);
  ...
  /* Bug #40123 - Fix bus error logging APIs to work on per device basis */
  g_pTEN->debug_base_address = (cs_uint32)(*BASE_ADDR);
  g_pTEN->debug_dev_id = DEV_ID;
...
cs_status TEN_MOD_COMMON_VALIDATE_INTERNAL(cs_uint16 MODULE_ID,  T41_t **BASE_ADDR)
{
  ...
  *BASE_ADDR = ((T41_t *)(g_pTEN->p_dev_cb_tbl[temp_dev_id]->base_addr));
  ...
  /* Bug #40123 - Fix bus error logging APIs to work on per device basis */
  g_pTEN->debug_base_address = (cs_uint32)(*BASE_ADDR);
  g_pTEN->debug_dev_id =  temp_dev_id;
...
/****************************************************************/
{
  ...
  /* Bug #40123 - Fix bus error logging APIs to work on per device basis */
  g_pTEN->debug_base_address = dev_base;
  g_pTEN->debug_dev_id = dev_id;
...
/****************************************************************/
{
  ...
  /* Bug #40123 - Fix bus error logging APIs to work on per device basis */
  return (g_pTEN->ten_bus_error_count[dev_id]);
}
...
/* RETURNS    : CS_OK or CS_ERROR                               */
/* DESCRIPTION:                                                 */
/* Enables or disables the checking of bus errors since last    */
/* ten_dev_init call.                                           */
/* This is provided mainly for debugging purposes.              */
/*                                                              */
