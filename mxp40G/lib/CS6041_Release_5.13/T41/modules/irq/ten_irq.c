 ...
 *
 * $Id: ten_irq.c,v 1.14 2014/04/03 20:18:48 jccarlis Exp $
 *
 */
...
cs_status ten_dev_irq_enable_disable(cs_uint16 dev_id, ten_irq_node_ptr node, cs_uint16 enable_bits, cs_uint16 disable_bits, ten_irq_tree_dir_t dir)
{
    ...
    /* Bugzilla #42300 - Klocwork warnings fixed */
    cs_uint16             enable_reg_data = 0;
    cs_boolean            enable;
    cs_uint16             i;
    ...
    if (node->parents != TEN_IRQ_NODE_NO_NODE)
    {
        ...
        /* Bug #41553: ten_dev_irq_disable() with dir = T41_IRQ_DIR_UP will disable all the interrupt nodes up the tree even if other interrupts are enabled */
        if (((enable == TRUE) || ((enable == FALSE) && (enable_reg_data == 0))) && (dir == TEN_IRQ_DIR_UP || dir == TEN_IRQ_DIR_BOTH))
        {
...
cs_status ten_dev_irq_clear_node(cs_uint16 dev_id, ten_irq_node_ptr node)
{
    ...
    for (i=0; i<node->slices; i++)
    {
        ...
        printf("ten_dev_irq_clear: Clearing 0x%05X: %s\n", reg_addr, node->name);
        TEN_DEV_UNLOCK(dev_id, TEN_ID_IRQ);
...
cs_status ten_mod_irq_enable_disable(cs_uint16 module_id, cs_uint8 slice_in, ten_irq_node_ptr node, cs_uint16 enable_bits, cs_uint16 disable_bits, ten_irq_tree_dir_t dir)
{
   ...
   cs_char8 *func = "ten_mod_irq_enable_disable";  
    ...
    if (node->parents != TEN_IRQ_NODE_NO_NODE)
    {
        /* Bug #41553: ten_mod_irq_disable() with dir = T41_IRQ_DIR_UP will disable all the interrupt nodes up the tree even if other interrupts are enabled */
        if (((enable == TRUE) || ((enable == FALSE) && (enable_reg_data == 0))) && (dir == TEN_IRQ_DIR_UP || dir == TEN_IRQ_DIR_BOTH))
        {
...
/*
                ...
                printf("PROCESSING node->child_name[i] '%s'\n", node->child_name[i]);
*/
            ...
            {
/*
                printf("PROCESSING intermediate node\n");
*/
                /* children is a variable-length array; check the index against the length */
                if (i < node->num_children)
                {
