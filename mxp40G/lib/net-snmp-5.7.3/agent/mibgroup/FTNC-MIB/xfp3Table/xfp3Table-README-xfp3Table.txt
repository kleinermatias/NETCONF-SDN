...
------------------------------------------------------------------------
  ...
  the xfp3Table table.  The agent will start by looking for the right
  ...
  file defaults/table-xfp3Table.m2d, and then re-running mib2c.
    ...
    will be generated:
      ...
      xfp3Table_allocate_data
      xfp3Table_release_data
 ...
 Index(es) for the xfp3Table table
  ------------------------------------------------------------
  The index(es) for the xfp3Table table are:
...
************************************************************************
xfp3Table File Overview
------------------------------------------------------------------------
  Several files have been generated to implement the xfp3Table
  table. We'll go through these files, one by one, explaining each and
  letting you know which you need to edit.


File: xfp3Table_data_access.[c|h]
------------------------------------------------------------------------
  The xfp3Table_data_access file contains the interface to your data in
  ...
  FUNC : xfp3Table_indexes_set
  WHERE: xfp3Table_data_access.c
...
------------------------------------------------------------------------
  ...
  -------------------
  TODO : retrieve column data from raw data
  FUNC : xfp3LowTxPow_get

  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : xfp3HighTxPow_get

  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : xfp3LowRxPow_get

  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : xfp3HighRxPow_get

  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : xfp3RxCdrLol_get

  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : xfp3TxCdrLol_get

  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : xfp3LasFault_get



File: xfp3Table_data_set.c
------------------------------------------------------------------------
...
NOTE: Depending on your configuration, some of the functions used in the
      ...
      were generated for the xfp3Table table.

To watch the flow of the xfp3Table table, use the
...
e.g.
        snmpd -f -Le -Dxfp3Table,verbose:xfp3Table,internal:xfp3Table
...
/*---------------------------------------------------------------------
 * FTNC-MIB::xfp3Entry.xfp3Index
 * xfp3Index is subid 1 of xfp3Entry.
 * Its status is Current, and its access level is NoAccess.
 * OID: .1.3.6.1.4.1.43484.2.5.1.1
 * Description:
 ...
 *
 *
 * NOTE: NODE xfp3Index IS NOT ACCESSIBLE
 *
 *
 */
/*---------------------------------------------------------------------
